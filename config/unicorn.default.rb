...
# The port to serve the website on. It's best to use a server such as Nginx to proxy
# all calls from port 80 to this port.
#listen 7000

# The Unix socket to listen to for incoming connections. Unix sockets are recommended
# as they're much faster than listening to a port. The value of this option should be
# the full path to the socket.
listen 'path/to/socket.sock'

##
# The user and user group to use for the application. It's recommended to assign a 
# separate user to the website so that in the event of a system breach the hacker
# (or the application itself) can't nuke your server.
#
user 'USER', 'GROUP'
